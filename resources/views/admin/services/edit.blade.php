<x-app-layout>
    <div class="max-w-5xl mx-auto py-12 px-4 sm:px-6 lg:px-8 bg-slate-50/50 min-h-screen">
        ...
        {{-- Navigation Header --}}
        <div class="mb-8 pl-1">
            <a href="{{ route('admin.services.index') }}" class="text-sm font-bold text-slate-400 hover:text-slate-800 transition flex items-center gap-2 group mb-3 w-fit">
                <svg class="w-4 h-4 transform group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                </svg>
                Kembali ke Layanan & Harga
            </a>
            <h2 class="text-3xl font-black tracking-tight text-slate-800">Ubah Data Layanan</h2>
            <p class="text-slate-400 text-sm mt-0.5">Perbarui informasi nama, kategori jenis, atau tarif jasa Clean Time.</p>
        </div>
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 md:p-10 max-w-3xl">
            <form id="serviceForm" action="{{ route('admin.services.update', $service->id) }}" method="POST" class="space-y-6">
                ...
                <div class="group">
                    ...
                    <input type="text" id="service_name" name="service_name" value="{{ old('service_name', $service->service_name) }}" required autofocus placeholder="Contoh: Cuci Kering Setrika Premium"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="group">
                        ...
                        <div class="relative">
                            <select id="unit" name="unit" required 
                                ...
                                <option value="Kg" {{ old('unit', $service->unit) == 'Kg' ? 'selected' : '' }}>KILOAN (Per Kg)</option>
                                <option value="Pcs" {{ old('unit', $service->unit) == 'Pcs' ? 'selected' : '' }}>SATUAN (Per Pcs)</option>
                            </select>
                    ...
                    <div class="group">
                        ...
                        <div class="relative mt-2">
                            ...
                            <input type="text" id="price_display" value="{{ number_format(old('price', $service->price), 0, ',', '.') }}" placeholder="0" required
                            ...
                            <input type="hidden" name="price" id="price_actual" value="{{ old('price', $service->price) }}">
                        </div>
                        @error('price') 
                            <span class="text-rose-500 text-[11px] mt-2 ml-1 block font-semibold">{{ $message }}</span> 
                        @enderror
                ...
                <div class="pt-4">
                    <button type="submit" id="submitBtn"
                            class="w-full bg-slate-800 text-white py-4 rounded-2xl font-extrabold shadow-lg shadow-slate-900/10 hover:bg-slate-900 transition-all active:scale-[0.99] text-xs tracking-widest uppercase flex items-center justify-center gap-2 group">
